...
std::vector<OpSpec> makeTable() {
    ...
    t.push_back(
        OpSpec{"stroke",
               ...
               {layerParam(),
                P{"points", "array", true,
                  "[{x,y,p?}...] 또는 [[x,y]...]. p 는 필압 0..1(없으면 pressureProfile)"},
    ...
    t.push_back(OpSpec{"transform",
                       "draw",
                       "레이어를 **정수 픽셀 단위로 평행이동**한다",
                       true,
                       "",
                       true,
                       ViewMode::Dirty,
                       {layerParam(), P{"dx", "int", false, "가로 이동(px)"},
                        P{"dy", "int", false, "세로 이동(px)"},
                        P{"scale", "number", false, "미지원. 주면 거절한다"},
                        P{"rotate", "number", false, "미지원. 주면 거절한다"}},
                       &ops::transform});

    // ── 선택 ─────────────────────────────────────────────────────────────
    t.push_back(OpSpec{"select",
                       "select",
                       "선택 영역을 정한다(지금 모델은 **사각형 하나**다)",
                       true,
                       "",
                       false,
                       ViewMode::None,
                       {P{"region", "region", false, "없으면 선택을 해제한다"},
                        P{"mode", "string", false, "rect | lasso | color — rect 만 지원"}},
                       &ops::select});
    t.push_back(OpSpec{"select.invert",
                       "select",
                       "선택을 반전한다",
                       false,
                       "선택 모델이 사각형 하나뿐이라 반전 결과를 담을 수 없다. "
                       "선택 마스크가 들어오면 그때 연다(docs/04)",
                       false,
                       ViewMode::None,
                       {},
                       &ops::selectInvert});
    t.push_back(OpSpec{"select.expand",
                       ...
                       false,
                       ViewMode::None,
                       {P{"by", "int", true, "px. 음수면 줄인다"}},
                       &ops::selectExpand});
    t.push_back(OpSpec{"select.feather",
                       "select",
                       "선택 경계를 부드럽게 한다",
                       false,
                       "부드러운 선택을 담을 마스크 저장소가 아직 없다. 있는 척하지 않는다",
                       false,
                       ViewMode::None,
