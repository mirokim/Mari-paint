...
enum class FrameFlag : u32 {
    ...
    Eraser = 1u << 3, ///< 지우개 모드
    /// 🔴 붓질이 **아니다**(docs/06 결정 ①). fill·erase·gradient 처럼 영역을 직접 쓰는
    ///    연산이 남기는 합성 프레임에 선다. 두 개가 한 쌍으로 나가고(Down|Synthetic 좌상단,
    ///    Up|Synthetic 우하단) 두 점이 영향 영역을 **프레임 바이트 안에** 담는다.
    ///    비트가 서명될 정본 안에 있으므로 사후에 지워 붓질로 둔갑시킬 수 없다.
    ///    크기·오프셋은 그대로다 — flags 는 이미 u32 이고 비트 4는 비어 있었다.
    Synthetic = 1u << 4,
};
