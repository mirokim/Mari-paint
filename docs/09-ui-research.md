...
| `Ctrl+T` | 자유 변형(이동·확대·회전·뒤집기, Enter/Esc, 화살표 1px) | PS |
| `Ctrl+U` / `Ctrl+L` / `Ctrl+M` / `Ctrl+I` / `Ctrl+Shift+U` | 색조·채도 / 레벨 / 곡선 / 반전 / 채도 제거 | PS |
| `Ctrl+Alt+I` / `Ctrl+Alt+C` | 이미지 크기 / 캔버스 크기 | PS |
| `U` / `Shift+U` / `Shift+G` | 직선 / 사각형 / 그라데이션 도구 (타원은 도구상자) | PS(U 는 도형, G 는 그라데이션 계열) |
| `Ctrl+'` / `Ctrl+Alt+S` | 격자 / 좌우 대칭 그리기 | PS(격자) · 변형 |
