...
`io/psd`: 읽기/쓰기 — v1 · RGB 8비트 · 레이어 RGBA + 사용자 마스크(-2 채널) · RLE/Raw · 그룹(lsct 1/2/3) · 클리핑 · 불투명도 ·
가시성 · 합성 모드 18종 · 이름(luni 유니코드) · 합성 이미지. 안 하는 것: PSB · 16/32비트 · CMYK · 조정/텍스트/스마트 레이어 ·
레이어 효과(경고로 남긴다). Pillow 로 우리 PSD 를 열어 합성·레이어 목록이 맞는 것을 확인. 파일 대화상자와 `doc.open/save`
(확장자로 포맷 선택)에 연결. prooflog 는 .ora 에만 담긴다 — PSD 는 교환용.

### 11.7 헤드리스 페인팅 실증 (2026-09-19)

`examples/meadow.py` → 7,247 연산, 실패 0. 배운 규칙은 docs/11.

### 11.8 코드리뷰 라운드 (2026-09-19)

`5e3bcc3..HEAD` 리뷰 11건 → `c22d2ab` 에서 전부 수정(변형 undo 안전장치 · 캔버스 연산 롤백/잠긴 레이어 동행 · 백킹 재할당 ·
에어브러시 hold 마지막 점 + 내장 프리셋 플래그 · PSD 사각형 검증 · ungroup 활성 · 브러시 가져오기 중복 · 대칭 undo 묶기 ·
복구본 보존). 테스트 62건.

## 12. 나중에 생각할 것 — 생성형 이미지 API 연결 (메모, 2026-09-19)

GPT Image(gpt-image-1) · Gemini 이미지(nano banana) 계열을 **Mari 안의 도구 하나**로 넣어, AI 가 페인팅과 이미지 편집을
같은 문서 안에서 하게 한다. 외부에서 통째로 그려 붙이는 것보다 정밀해야 한다는 뜻: 결과는 항상 레이어·마스크·기록을 거친다.

- 에이전트 op: `ai.generate` · `ai.edit` · `ai.inpaint`(선택 = 마스크) · `ai.describe`(캔버스를 보고 답). 결과는 새 레이어 또는
  선택 안 클리핑, `RegionOpKind::AiImage` 로 기록, 실행취소 한 번.
- `ImageProvider { generate, edit(image, mask, prompt), vision }` 추상화 + OpenAI/Gemini 구현. 키는 환경변수/설정 파일, 로그 마스킹.
- "더 잘 보기": `render` 의 부분 크롭·레이어별 렌더를 비전 입력으로 → 돌아온 판단을 브러시/선택/보정 op 로 실행하는 루프.
- 네트워크는 Qt 없이 `agent/` 아래(WinHTTP 또는 libcurl) — 헤드리스에서도 돌아야 한다.
- GUI: 프롬프트 패널 + 결과 레이어 미리보기/적용/취소.
- 결정 보류: 어느 프로바이더부터, 키 관리 방식, 비용/속도 제한.
