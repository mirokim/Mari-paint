...
> **판단:** 대부분의 앱처럼 **환경설정에서 선택**하게 하되, **런타임 자동 감지 + 폴백**을 넣는다.
> "필압이 안 먹어요"는 페인트 툴에서 제일 흔한 불만이고, 여기서 갈린다.
...
1. **sigan 앱 연동 규격** — sigan이 Mari Paint에게 무엇을 시키고 싶은지 정해야 COM 인터페이스가
   확정된다. (예: 캔버스 열기? 레이어 주입? 렌더 결과 회수? 실시간 스트로크 전송?)
